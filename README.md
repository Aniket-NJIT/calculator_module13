...
* **Interactive Frontend UI:** Client-side HTML/JS application (`index.html`, `login.html`, `register.html`) providing a seamless interface for users to register, log in, compute math operations, and view their calculation history.
* **JWT-Based Security:** Secure user registration and login endpoints utilizing JSON Web Tokens (JWT) for session management, alongside `passlib` and `bcrypt` for secure password hashing.
* **Calculator BREAD API:** Complete backend endpoints to Browse, Read, Edit, Add, and Delete mathematical operations. Users can only access and modify their own calculation history.
...
* **Comprehensive Testing:** Unit, integration, and E2E testing using `pytest` and Playwright.
...
```bash
git clone https://github.com/Aniket-NJIT/calculator_module13.git
cd calculator_module13
```
...
- test_e2e.py: Playwright tests that simulate user registration, UI validation, and login flows on the frontend.
...
View the docker hub repository at: https://hub.docker.com/r/akhalate/calculator-frontend

You can pull the latest built image directly from Docker Hub to run the application anywhere:
```bash
docker pull akhalate/calculator-frontend
```
...
docker run -p 8000:8000 \
  ...
  akhalate/calculator-frontend:latest
```
